...
Before the launch, it captures the current process list.

After a configurable delay (default 15 seconds), it captures the list again.

It computes the diff: processes that appeared during the window.

It filters out known launcher shells (steam.exe, EpicGamesLauncher.exe, browsers, OS helpers, antivirus processes).

It prioritizes candidates that descend from the launcher shell or from the specific process that was launched.

It picks the largest remaining candidate by memory footprint.

The result is stored in the Tag Library so future taps kill instantly without needing to re-detect.
...
Download or clone the repository to a folder, e.g. C:\TagToolSH.

Double-click run.bat.

On first launch, run.bat will download a portable Python 3.12 embeddable runtime into app_data/python/, bootstrap pip inside that runtime, and install PySide6, pyscard, ndeflib, and psutil into app_data/libs/.

The app window opens.

To make it portable, copy the entire TagToolSH folder — including app_data/ — to a USB stick. It runs from any folder on any Windows machine.
...
Open the Tag Studio / Writer tab.

Pick an action type: Spotify, Steam, Direct Executable, Web URI, or Custom Text.

Fill in the target. For Spotify, paste a spotify:track:... URI or an https://open.spotify.com/... link. For Steam, enter the App ID (e.g. 730) or a steam://rungameid/... URI. For Executable, click Browse and pick a .exe, .bat, .cmd, or .lnk. For Web, paste a URL or custom protocol URI.

Optionally tick Kill launched process when this tag is removed.

Click Write to NFC Tag.

Place an NTAG on the reader. The reader writes and verifies the data.
...
Reading Tags
The Live Scanner & Launcher tab shows the UID of the current tag with a Copy UID button, the tag type (NTAG213/215/216, MIFARE Ultralight, etc.) and memory size, the detected action with a color-coded badge, and a recent activity log with timestamps.

Buttons at the bottom: Launch Action Now for a manual one-shot execution, Save to Tag Library to assign a custom action to this UID, and Encode / Overwrite Tag to jump to the writer tab with this tag's data pre-filled.
...
Global setting (Terminate application on tag removal in Settings). If off, no tag is ever killed.

Tag's own flag — either the NDEF kill=1 record or the Tag Library's kill_on_removal field.

Hard exclusions — Spotify actions are never tracked or killed.
...
Before the URI fires (or the wrapper runs), the app takes a snapshot of every running process — PID, image name, parent PID, and memory usage.

The action is launched.

A background thread waits for the configured delay (default 15 s).

A second snapshot is taken.

The difference between the snapshots is computed: processes that appeared during the window.

Filtering: a blocklist removes known launcher shells, browsers, OS helpers, antivirus, and system processes. Candidates must be a descendant of the launched process's PID (strict) or a descendant of a known launcher shell (fallback). A memory threshold (50 MB) rejects tiny helper processes.

The largest surviving candidate is chosen.

The result is written to the Tag Library under auto_detected_process.

On tag removal, the app runs taskkill /F /IM <name> /T.
...
Closing the window: the X button opens a modal with three choices. Minimize to Tray hides the window and keeps the app running. Exit Launcher shuts down cleanly. Cancel returns to the window.

Tray icon menu: Open Launcher Window restores the window. Exit quits immediately, no modal.
...
Enable Action Execution on Tag Tap (default: On) — master toggle for automatic action firing.

Read-Only Mode (default: Off) — strongest safety; nothing launches.

Ask for confirmation before launching .exe (default: Off) — modal prompt before executables.

Terminate application on tag removal (default: Off) — master kill switch.

Also terminate all tag-launched processes when the app exits (default: Off) — cleans up any tracked processes on app exit.

Tag Debounce / Hold Cooldown (default: 2.5 s) — delay before the same UID can fire again.

Launcher Auto-Detection

Auto-detect game process for Steam / Epic / URI launcher tags (default: On) — enables the snapshot-diff detection.

Auto-detection delay (default: 15 s) — time to wait before comparing process lists.

BSOD Effect

Show bsod.png fullscreen after kill (default: On) — enables the easter egg.

Auto-close after (default: 60 s) — safety timeout.

ACR122U Hardware Feedback

Sound hardware buzzer on tag tap (default: On) — one beep per successful read.

Sound hardware buzzer on write success (default: On) — one beep per successful write.

Three rapid beeps always play on write failure, regardless of the setting.

System Tray & Windows Startup

Minimize to system tray when minimized or closed (default: On) — enables tray behavior.

Start with the main window maximized (default: On) — opens maximized.

Launch automatically on Windows startup (default: Off) — registry-based autostart.
...
MIFARE Classic 1K/4K — not NDEF-addressable. Use UID mapping only.

Any ISO 14443-A card — UID mapping via the Tag Library.
...
Prerequisites
Python 3.12 installed on the build machine. 3.13 may work; 3.14 is not recommended because PySide6's wheel availability can be spotty.

The project's virtual environment activated. On a fresh clone, run these commands in cmd:

py -3.12 -m venv .venv
.venv\Scripts\activate
pip install PySide6 pyscard ndeflib psutil pyinstaller

A .ico file at app_data/icon.ico (used for both the .exe icon and the runtime window icon). It must be a real multi-resolution ICO, not a PNG renamed. If you don't have one, remove the icon= line from the spec and the app will fall back to a vector-drawn icon.

A bsod.png at the project root or in app_data/ if you want the BSOD effect.
...
Common causes: you lifted the tag before the delay finished (keep the tag on the reader for the full duration shown in the status bar); the game uses a .bat launcher that spawns with start (Windows detaches the game from the launcher, breaking the ancestry link — set the manual process_to_kill for that tag); the candidate is under 50 MB (small games or utilities may be filtered — set the manual override).
...
The kill command uses image name, not PID, unless the tag mapping explicitly uses a manual PID. This means taskkill /IM affects all processes with that image name — if you launched the same game manually, lifting the tag will kill that too.
...
The application is bundled with the following third-party libraries, each under its own license: PySide6 (LGPL v3) for Qt bindings, pyscard (LGPL v2.1) for PC/SC smart card access, ndeflib (MIT) for NFC Forum NDEF parsing, and psutil (BSD 3-Clause) for cross-platform process inspection.
